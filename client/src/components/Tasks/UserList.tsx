...
import { useGetTeamListQuery } from "../../redux/slices/api/userApiSlice.ts";

const UserList = ({ setTeam, team }:any) => {
  ...
  const handleChange = (el:any) => {
    setSelectedUsers(el);
    setTeam(el?.map((u:any) => ({id:u.id})));
  };
  useEffect(() => {
    if (team?.length < 1) {
      data && setSelectedUsers([data[0]] as any);
    } else {
        ...
        <div className='relative mt-1'>
          <ListboxButton className='relative w-full cursor-default rounded bg-white pl-3 pr-10 text-left px-3 py-2.5 2xl:py-3 border border-gray-300 sm:text-sm'>
            <span className='block truncate'>
              {selectedUsers?.map((user:any) => user.name).join(", ")}
            </span>
